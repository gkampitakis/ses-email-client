...
class SESTransporter {
  constructor (production, cacheSize, accessKeyId, secretAccessKey, region) {
    const settings = {
      ...(accessKeyId && secretAccessKey && { credentials: new Credentials({ accessKeyId, secretAccessKey }) }),
      ...(region && { region })
    };

    if (typeof production !== 'boolean' || !cacheSize) throw new Error('SESTransporter is not initialized correct');

    if (Object.keys(settings).length) config.update(settings);

    this.client = new SES();
    this[prod] = production;
    this[LRU] = HLRU(cacheSize);
  }
  ...
  async [processAttachments] (files) {
    ...
    for (const file of files) {
      ...
      let attachment = this[LRU].get(filename);
      ...
      attachments.push(attachment);
      this[LRU].set(filename, attachment);
    }
