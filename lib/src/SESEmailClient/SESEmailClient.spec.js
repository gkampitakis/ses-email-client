...
describe('SESEmailClient', () => {
  ...
  const { getSpy, setSpy, HLRUConstructorSpy } = jest.requireMock('hashlru');
  let { mockCache } = jest.requireMock('hashlru');
  ...
  describe('When call send method', () => {
    ...
    it('Should cache templates in production', async () => {
      const client = new SESEmailClient({
        production: true,
        templateLanguage: 'ejs'
      });
